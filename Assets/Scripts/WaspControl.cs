...
public class WaspControl : MonoBehaviour
{
 ...
 //       sound = GetComponent<AudioSource>();
    ...
    void Update()
    {
        if (isRun && Time.timeSinceLevelLoad > Configuration.start_pause)
        {
    ...
    {
        Debug.Log("Wasp Trigger tag:" + collider.tag);
        isRun = false;
        if (collider.tag == "LadyBug")
        {
            GameObject.FindGameObjectWithTag("AudioFail").GetComponent<AudioInterScene>().PlayMusic();
            //           sound.Play();
            //set velocity 0
